...
- [x] Publicação confirmada: commit funcional `4909185`, push main; home deployment `dpl_J7htsnnotfT6JCa5oJov11pwLurB` READY no mesmo commit. HTTPS home/asset retornaram 200, documento inclui configuração nova e asset contém a chave pública fornecida (somente booleano de igualdade registrado, sem repetição do valor). Nenhuma function/DNS/integração legada alterada nesta rodada.

## Cartão aprovado e dashboard demonstrativo Discordia — 16/09/2026

- [x] Consulta restrita ao cliente de teste confirmou cartão approved, pedido live, um payment_event e um projeto inicialmente ROADMAP_PAGO. A entrega real ocorreu; replay autenticado continua pendente para fechar C3 integralmente. Nenhuma cobrança adicional foi criada pelo agente.
- [x] Conta de teste já existia e estava confirmada, ainda sem papel. Primeira liberação pelo NO_ADMIN retornou 502; log sanitizado da função identificou email_exists. Corrigido o fluxo: conta confirmada recebe magiclink, enquanto cadastro novo/não confirmado mantém invite. Conflito com papel NO_ADMIN é recusado antes de gerar link. Senha existente não é redefinida.
- [x] Harness Skill 01: 25/25 assertions, incluindo conta confirmada, senha preservada, redirect direto ao projeto, RLS restrita ao próprio projeto e NO_ADMIN sem rebaixamento/membership. Vitest 152/152, incluindo três novos testes do case; lint, TypeScript/build e validação do JSON aprovados. Provas de banco/E2E anteriores não foram reexecutadas sem mudança de schema/React.
- [x] Correção publicada somente em skill-01-ativar-dashboard, mantendo verify_jwt e autorização NO_ADMIN. Segunda liberação pelo painel administrativo real funcionou. Consulta confirmou JANELA_DE_DECISAO, INICIAL_15_DIAS, uma membership e uma atividade de ativação; janela iniciada às 15:15:43 de São Paulo, sem retroagir D+1/D+3.
- [x] Conteúdo reutilizável em app/scripts/exemplos/roadmap-discordia-demo.json, sem PII: case existente Discordia; três planos fictícios (R$ 1.000/7 dias, R$ 2.500/15 dias, R$ 5.000/30 dias), com avisos explícitos de ausência de oferta/contrato/cobrança. Nenhuma conta, código ou configuração do Discordia foi alterada.
- [x] Interface de produção inspecionada com sessão NO_ADMIN: três cards, preços e avisos renderizados; iframe carregou a tela real de entrada do Discordia; aba Etapas exibiu os dois itens originais. Estado confirmado dos módulos: como_funciona/prototipo/etapas ativos; editor/versoes/marca bloqueados. Não houve conversão, preferência de tier ou contratação.
- [ ] Login de produção com a própria conta CLIENT ainda depende do titular; inspeção administrativa da interface não substitui essa prova. Cliente deve usar a senha que já definiu e iniciar sessão nova para atualizar o papel CLIENT.
