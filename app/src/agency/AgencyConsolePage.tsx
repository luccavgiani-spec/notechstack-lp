...
function Versions({ detail, expanded = false }: { detail: AgencyDetail; expanded?: boolean }) {
  ...
  return versions.length ? <ol className="ac-timeline">{versions.map(v => <li key={v.id}><span className="ac-timeline-icon"><Icon name="file" /></span><div><strong>{v.label}{v.current ? <span className="ac-current">Atual</span> : null}</strong><p>{v.changelog}</p></div><time>{dateLabel(v.publishedAt)}</time></li>)}</ol> : <div className="ac-empty"><Icon name="layers" /><p>Nenhuma versão publicada.</p><span>As entregas aparecerão aqui conforme forem publicadas.</span></div>
}
function ProjectInfo({ project, version }: { project: AgencyProject; version?: string }) {
  const entries: [string, ReactNode][] = [['Cliente', project.clientName], ['Projeto', project.name], ['Próxima entrega', dateLabel(project.nextDate)], ['Status', <Status value={project.status} />], ['Versão atual', version ?? 'Ainda não publicada'], ['Entregas concluídas', `${project.doneTasks} de ${project.totalTasks}`], ['Última atualização', dateLabel(project.updatedAt)]]
...
function ProjectDetail({ agencyId, project, toolbar }: { agencyId: string; project: AgencyProject; toolbar: ReactNode }) {
  ...
  return <><header className="ac-detail-header"><div className="ac-container"><div className="ac-topbar ac-topbar-dark"><Link to={`/agencia/${agencyId}`} className="ac-logo"><img src="/no-tech-stack-branca-ponto-ambar.svg" alt="nó tech stack" /></Link><nav className="ac-breadcrumb" aria-label="Navegação do projeto"><Link to={`/agencia/${agencyId}`}>Clientes</Link><span>›</span><span>{project.clientName}</span></nav><div className="ac-topbar-actions">{toolbar}</div></div>
    ...
    {error ? <div className="ac-message" role="alert">Não foi possível abrir este projeto. Confira seu vínculo com a agência.<button className="ac-button" onClick={() => { setError(false); setRevision(r => r + 1) }}>Tentar novamente</button></div> : !detail ? <p className="ac-message" role="status">Carregando projeto…</p> : <>{tab === 'stages' ? <><section className="ac-board" aria-label="Etapas e entregas">{columns.map(column => {
      const tasks = detail.tasks.filter(t => t.status === column.status)
      return <section className={`ac-column ac-column-${column.tone}`} key={column.status}><h2><i className="ac-dot" />{column.title}<span>({tasks.length})</span></h2><div className="ac-task-list">{tasks.map(task => <article className="ac-task" key={task.id}><div className="ac-task-title">{task.status === 'concluido' ? <span className="ac-task-check"><Icon name="check" /></span> : null}<h3>{task.title}</h3></div>{task.phase ? <span className="ac-task-tag">{task.phase}</span> : null}{task.date ? <p className="ac-task-date"><Icon name="calendar" />{dateLabel(task.date)}</p> : null}</article>)}{!tasks.length ? <p className="ac-column-empty">Nenhuma tarefa nesta etapa.</p> : null}</div><div className="ac-column-foot">{tasks.length} {tasks.length === 1 ? 'tarefa' : 'tarefas'}</div></section>
