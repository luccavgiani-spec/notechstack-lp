...
describe('F4-13 biblioteca', () => {
  ...
  it('C9 envia os filtros da biblioteca para a busca', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Stack'), { target: { value: 'react' } })
    fireEvent.click(screen.getByLabelText('Pode reutilizar'))
    fireEvent.click(screen.getByLabelText('Autorizado para case'))
    await waitFor(() => expect(mocks.listArchiveAssets).toHaveBeenLastCalledWith({ tags: { nicho: ['saude'], stack: ['react'] }, reusableOnly: true, caseOnly: true }))
  })
})
