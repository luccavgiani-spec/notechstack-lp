...
it('shows the agreed delivery window without inventing a published version', async () => {
  mocks.loadAgency.mockResolvedValue({ agency, projects: [{ ...project, startedOn: '2026-07-22', deliveryFrom: '2026-09-09', deliveryTo: '2026-09-23', deliveryNote: '7–9 semanas', reviewLabel: 'Versão final em análise' }] })
  show('/agencia/maisis/projetos/gazeta')
  await screen.findByText('Nenhuma tarefa cadastrada.')
  await userEvent.click(screen.getByRole('button', { name: 'Informações' }))
  expect(screen.getByText('22/07/2026')).toBeVisible()
  expect(screen.getByText('09/09/2026 a 23/09/2026')).toBeVisible()
  expect(screen.getByText('7–9 semanas')).toBeVisible()
  await userEvent.click(screen.getByRole('button', { name: 'Versões e entregas' }))
  expect(screen.getByText('Nenhuma versão publicada.')).toBeVisible()
})
